...
export const Tokens = ((): Record<string, ReturnType<typeof createToken>> => {
  ...
  const regConcat = (...rs: (RegExp | string)[]) =>
    new RegExp(rs.map((r) => (r instanceof RegExp ? r.source : r)).join(""));
  ...
  Tokens.TimestampCompact = createToken({
    name: "TimestampCompact",
    pattern: /\d{8}T?\d{4}(?:\d{2}(?:\.\d+)?)?(?:Z|[+-]\d{2}:?\d{2})?/,
    categories: [Tokens.Timestamp],
  });
  Tokens.TimestampISO8601 = createToken({
  ...
  (() => {
    ...
    Tokens.StringLiteralDouble = createToken({
      name: "StringLiteralDouble",
      pattern: /"(?:[^\\"]|\\(?:[bfnrtv"\\/]|u[0-9a-fA-F]{4}|u\{[0-9a-fA-F]+\}|x[0-9a-fA-F]{2}|[0-7]{1,3}|.))*"/,
      categories: [Tokens.StringLiteral],
    });
    Tokens.StringLiteralSingle = createToken({
  ...
  // ログレベル
  Tokens.LogLevel = createToken({
    name: "LogLevel",
    pattern: /\b(?:DEBUG|INFO|WARN|WARNING|ERROR|FATAL|TRACE|debug|info|warn|warning|error|fatal|trace)\b/,
  });
